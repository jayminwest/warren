#!/usr/bin/env bun
/**
 * Validate that the commands and file references in AGENTS.md still resolve.
 * AGENTS.md is the only target: CLAUDE.md is a symlink to it (warren-b771),
 * so validating both would check the same bytes twice. Static check — no
 * commands are executed here, since the test/lint/typecheck gates are already
 * exercised by the rest of the CI pipeline (.github/workflows/ci.yml).
 *
...
const KNOWN_MISSING_PATHS = new Set<string>([
	...
	"defaults.json", // legacy per-project config file, written at runtime
	".mulch/archive", // runtime-created by `ml prune`, not checked in

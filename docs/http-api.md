...
| Method | Pattern | Handler | Notes |
| --- | --- | --- | --- |
| `GET` | `/events` | `eventsQueryHandler` | pl-7e38 step 15 (warren-5eec): the global events query for the Event explorer page. `readPublic` — each row rides the same per-event reduction `GET /runs/:id/events` applies (./events-query.ts), with internal kinds excluded in SQL. `?limit/?offset` capped at 500. |

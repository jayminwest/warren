 ...
 *
 * Parses the `ROUTE_TABLE` constant in `src/server/handlers.ts` and
...
function extractRouteTableBody(source: string): string {
	...
	if (!startMatch || startMatch[1] === undefined) {
		throw new Error(
			"ROUTE_TABLE not found in src/server/handlers.ts — has the extractor's regex drifted?",
		);
	}
	return startMatch[1];
}
...
export function renderMarkdown(routes: readonly Route[]): string {
	...
	sections.push("");
	sections.push("<!-- AUTO-GENERATED by `bun run gen:docs` from `src/server/handlers.ts`. -->");
	...
	sections.push(
		"This page enumerates every HTTP route registered by warren's `Bun.serve` " +
			...
			"[`src/server/handlers.ts`](../src/server/handlers.ts) so it can't drift " +
...
function main(): void {
	...
	if (checkMode) {
		...
		if (existing !== content) {
			console.error("docs/http-api.md is stale relative to src/server/handlers.ts.");

...
<!-- AUTO-GENERATED by `bun run gen:docs` from `src/server/handlers.ts`. -->
<!-- Do not edit by hand. CI fails if this file is out of sync. -->

This page enumerates every HTTP route registered by warren's `Bun.serve` router. It's derived directly from the `ROUTE_TABLE` array in [`src/server/handlers.ts`](../src/server/handlers.ts) so it can't drift from the running server.

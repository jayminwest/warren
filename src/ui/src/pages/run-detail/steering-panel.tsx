	...
	return (
		<section className="flex shrink-0 flex-col overflow-clip rounded-(--radius-md) border border-(--color-border) bg-(--color-surface)">
			<header className="flex h-[39px] shrink-0 items-center border-b border-(--color-border) px-3">
				<h2 className="text-sm font-semibold text-(--color-text)">Steering inbox</h2>
			...
			>
				<textarea
					 ...
					 */
					className="min-h-0 flex-1 resize-none rounded-(--radius-sm) border border-(--color-border-strong) bg-(--color-bg) px-2.5 py-2 text-sm text-(--color-text-2) placeholder:text-(--color-text-3) max-md:h-[36px] max-md:py-0 "
				/>
				<button
					type="submit"
					disabled={disabled || steer.isPending || body.trim().length === 0}
					className={cn(
						"inline-flex h-[31px] shrink-0 items-center rounded-(--radius-sm) px-[11px] text-sm  font-medium max-md:h-[36px]",
			...
			{steer.isError ? (
				<p className="px-3 pb-3 font-mono text-2xs text-(--color-danger)">
					{steer.error instanceof Error ? steer.error.message : String(steer.error)}
				</p>
			) : null}
			{success ? (
				<p className="px-3 pb-3 font-mono text-2xs text-(--color-success)">
	...
	return (
		<div className="flex flex-col items-end gap-1">
			<button
				...
				className={
					mobile
						? "font-mono text-sm  font-medium text-(--color-danger) disabled:opacity-50"
						: "inline-flex h-[31px] items-center gap-1.5 rounded-(--radius-sm) border border-(--color-border-strong) bg-(--color-surface) px-[11px] text-sm  font-medium text-(--color-danger) hover:bg-(--color-surface-hover) disabled:opacity-50"
				}
			>
			...
			{cancel.isSuccess && cancel.data !== undefined ? (
				<p className="font-mono text-2xs text-(--color-text-2)">

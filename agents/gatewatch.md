...
Every finding is recorded as a seed (step 5). The seed IS the delivery: the weekly warden-digest triages the accumulated open `audit`-labeled seeds directly — there is no separate warden conversation or message channel to feed. Lead each seed title with `gatewatch:` and include the SHA, the constitution article, and the file paths in the description (Article VIII evidence) so the digest can attribute and synthesize it. A fabricated or dropped seed is the only unrecoverable failure.

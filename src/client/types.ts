...
export interface RunRow {
	...
	/**
	 * Resolved workspace HEAD SHA (warren-b19e): the commit the workspace was
	 * actually at when reap measured `commitsAhead` — the base the run really
	 * sat on, vs `baseCommit`'s dispatch-time pin. Null = never measured.
	 */
	baseSha: string | null;
	/**
	...
	/** Per-run USD spend cap (warren-f8a2 / warren-b19e). Operator-only
	 * overlay on the runs list AND the detail GET; absent for spectators. */

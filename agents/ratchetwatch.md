...
Every finding is recorded as a seed or plan (steps 4–5). The seed IS the delivery: the weekly warden-digest triages the accumulated open `audit`-labeled seeds directly — there is no separate warden conversation or message channel to feed. Lead each seed title with `ratchetwatch:` and include the exact numbers (actual vs floor, file size vs limit, summed delta) in the description (Article VIII evidence) so the digest can attribute and synthesize it.

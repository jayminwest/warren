...
| Agent runtime | `AgentRuntimeAdapter` phase 1 — terminal detect, usage, error classes, seed layout | **Live** — phase 2 (harness repatriation) shipped with pl-3007 in v0.17.0. The adapters are warren-owned (`src/runtime/adapters/`). Remaining phase-1 slice: `runtimeId` typed off the union + the lint guard (GH#846 items 4–5, PR #964 in flight). |

## Now — in flight

- Nothing mid-campaign. The burrow absorption (pl-3007) and the self-host push both shipped in v0.17.0 — see Shipped. The last `AgentRuntimeAdapter` phase-1 slice (type `runtimeId` off the union + the lint guard, GH#846 items 4–5) is in flight as PR #964. The `IssueTracker` contract cut (Next item 1) queues next.

## Next — planned, in order

1. **`AgentRuntimeAdapter` phase 1 — the last slice.** Type `runtimeId` off `KNOWN_RUNTIME_IDS` at the contract seam, where `src/runtime/contract.ts` still says `string`. Add the lint guard: a runtime-id literal outside the adapter directory fails lint. Published as good-first-challenge GH#846. Items 1–3 shipped in v0.16.0 (PR #887), and items 4–5 are in flight as PR #964. Phase 2 (harness repatriation) shipped with pl-3007 — see Shipped.

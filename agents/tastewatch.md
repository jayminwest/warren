...
The digest seed IS the delivery: the weekly warden-digest triages the accumulated open `audit`-labeled seeds — including your digest — directly. Lead the seed title with `tastewatch digest:` (as above) so it is attributable. There is no separate warden conversation to feed; still report-only, still no dispatch or plan.
...
- No plans, no dispatch, no fixes, and no source edits. Report-only is your mandate; an attempt to exceed it is itself a constitution violation (Article IX). Filing your one digest seed is the whole of your delivery.
